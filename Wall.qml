  ...
  Column {
    ...
    Text {
      ...
      text: arcade.games.length === 0 ? "No ROMs found"
        : (arcade.searching ? "Nothing matches “" + arcade.filterText + "”" : Present.emptyNote(arcade.filters))
    ...
    Text {
      ...
      text: arcade.games.length === 0 ? "Press + (Alt+A) to add romsets, or drop them here"
        : (Browse.filtersActive(arcade.filters) ? (arcade.searching ? "Backspace to widen the search · " : "")
